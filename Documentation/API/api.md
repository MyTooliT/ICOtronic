---
substitutions:
  recommended amount of one or three enabled channels:
    "**recommended amount** of
    one or three enabled channels"
---
...
To connect to a sensor node (e.g. SHA, SMH, STH) use the async context manager of the coroutine {meth}`STU.connect_sensor_node`. To connect to a node you need to know one of the [identifiers of the node]. In the example below we connect to a node with the name `Test-STH`:
...
By default {attr}`StreamingData.values` contains 16-bit ADC values. To convert the data into multiples of g ([the standard gravity](https://en.wikipedia.org/wiki/Standard_gravity)) you can
...
If you want to store streaming data for later use you can use the {class}`Storage` class to open a context manager that lets you store data as [HDF5] file via the method {func}`add_streaming_data` of the class {class}`StorageData`. The code below shows how to store one second of measurement data in a file called `measurement.hdf5`.
...
[icolyzer package]: https://github.com/MyTooliT/ICOlyzer
[identifiers of the node]: https://mytoolit.github.io/ICOtronic/#sensor-node-identifiers
[recommended amount of one or three enabled channels]: https://mytoolit.github.io/ICOtronic/#channel-selection
[“measurement data”]: https://mytoolit.github.io/ICOtronic/#measurement-data
[“sampling rate”]: https://mytoolit.github.io/Documentation/#sampling-rate
...
For code examples, please check out the [examples directory](https://github.com/MyTooliT/ICOtronic/tree/main/icotronic/examples):

- [Read STH Name](https://github.com/MyTooliT/ICOtronic/tree/main/icotronic/examples/sth_name.py): Read the name of the „first“ available STH (sensor node id `0`).
- [Read Data Points](https://github.com/MyTooliT/ICOtronic/tree/main/icotronic/examples/read_data.py): Read five acceleration messages (5 · 3 = 15 values) and print their string representation (value, timestamp and message counter)
- [Store Data as HDF5 file](https://github.com/MyTooliT/ICOtronic/tree/main/icotronic/examples/store_data.py): Read five seconds of acceleration data and store it as HDF5 file
- [Concurrent Access to Sensor Node Data](https://github.com/MyTooliT/ICOtronic/tree/main/icotronic/examples/concurrent_access.py): Show how to read the current ADC configuration, while streaming is active
